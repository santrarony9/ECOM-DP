...
export class AvailabilityService {
   ...
   */
  async lockSlot(date: Date, startTime: string, endTime: string, bookingId: string): Promise<boolean> {
   ...
   */
  async releaseLock(date: Date, startTime: string, endTime: string): Promise<void> {
    const lockKey = this.generateLockKey(date, startTime, endTime);
    this.mockRedisLocks.delete(lockKey);
    this.logger.log(`Released lock: ${lockKey}`);
  }

  private generateLockKey(date: Date, startTime: string, endTime: string): string {
